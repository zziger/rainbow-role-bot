...
async function color () {
    client.guilds.forEach(async function (item1, number1) {
        if (client.guilds.get(item1) && client.guilds.get(item1).roles.find('name', 'Rainbow').editable)
        await colors.forEach(async function (item, number) {
            await setTimeout(async function () {client.guilds.get(item1).roles.find('name', 'Rainbow').setColor(item).catch(console.error);if(number === colors.length-1 && number1 === servers.length-1) setTimeout(function () {color().catch(console.error)}, 500)}, number*500);
        });
    });
}
client.on('ready', () => {
