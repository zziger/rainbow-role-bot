...
async function color () {
    client.guilds.forEach(async function (item1, number1) {
        ...
        await colors.forEach(async function (item, number) {
            await setTimeout(async function () {client.guilds.get(item1.id).roles.find('name', 'Rainbow').setColor(item).catch(console.error);if(number === colors.length-1 && number1 === client.length-1) setTimeout(function () {color().catch(console.error)}, 500)}, number*500);
        });
    });
}
client.on('ready', () => {
